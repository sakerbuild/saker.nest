...
@SakerTest
public class PartialIndexServerGetTasksExternalInfoTest extends ExternalScriptInformationTestCase {
	...
	private final class NestMetricImplementation extends BasicServerNestMetric {
		...
		public Integer getServerRequestResponseCode(String requesturl) throws IOException {
			...
			if ("https://testurl/tasks/index/six".equals(requesturl)) {
				return HttpURLConnection.HTTP_OK;
			}
			if ("https://testurl/tasks/index/sim".equals(requesturl)) {
				return HttpURLConnection.HTTP_OK;
			}
			if ("https://testurl/tasks/index/sev".equals(requesturl)) {
		...
		public InputStream getServerRequestResponseStream(String requesturl) throws IOException {
			...
			if ("https://testurl/tasks/index/six".equals(requesturl)) {
				return Files.newInputStream(getWorkingDirectory().resolve("taskindex/six.json"));
			}
			if ("https://testurl/tasks/index/sim".equals(requesturl)) {
				return Files.newInputStream(getWorkingDirectory().resolve("taskindex/sim.json"));
			}
			if ("https://testurl/bundles/index".equals(requesturl)) {
