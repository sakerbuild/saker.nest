...
@SakerTest
public class ExternalDependencyHashMainActionTest extends ManualLoadedRepositoryTestCase {
	//just a random uuid
	...
	protected void runTestOnRepo(SakerRepository repo) throws Exception {
		TreeMap<String, Set<Class<?>>> bundleclasses = TestUtils.<String, Set<Class<?>>>treeMapBuilder()//
				...
				.put("invalid.bundle-v1", ObjectUtils.newHashSet(SimpleMain.class))//
				.build();
		...
		assertException("saker.nest.exc.BundleDependencyUnsatisfiedException",
				() -> repo.executeAction(
						"main", "-Unest.server.offline=true", NestIntegrationTestUtils
								.createParameterBundlesUserParameter(bundleclasses.keySet(), bundleoutdir),
						"-bundle", "invalid.bundle-v1", "first-arg"));
	}
